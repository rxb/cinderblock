...
class IndexPage extends React.Component {
	...
	setBg(letter){
		...
		this.timeout = setTimeout(()=>{
			this.setState({bgColor: color, activeLetter: letter})
		}, 30)
	}

	resetBg(){
		this.timeout = setTimeout(()=>{
			this.setState({bgColor: rgbRed, activeLetter: null});
		}, 30)
	}
	...
	render(){
		return (
			<View>
				...
				<Stripe style={{backgroundColor: this.state.bgColor, minHeight: '75vh'}}>
							...
							<Section>
								<Chunk>
									<Text
										...
										>
											<RNText
												onMouseLeave={()=>this.resetBg()}
												>
												<RNText
													style={{opacity: ( !this.state.activeLetter || this.state.activeLetter == 'r') ? 1 : .4}}
													onMouseEnter={()=>this.setBg('r')}

													>r</RNText>
												<RNText
													style={{opacity: ( !this.state.activeLetter || this.state.activeLetter == 'g') ? 1 : .4}}
													onMouseEnter={()=>this.setBg('g')}
													>g</RNText>
												<RNText
													style={{opacity: ( !this.state.activeLetter || this.state.activeLetter == 'b') ? 1 : .4}}
													onMouseEnter={()=>this.setBg('b')}
													>b</RNText>
												<RNText
													style={{opacity: ( !this.state.activeLetter || this.state.activeLetter == 'work') ? 1 : .4}}
													onMouseEnter={()=>this.setBg('work')}
													>.&#8203;work</RNText>
											</RNText>
										</Text>
								</Chunk>
